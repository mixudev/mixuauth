...
class OtpService
{
    ...
    */
     ...
     */
    public function generateOtp(User $user, string $ipAddress, string $deviceFingerprint, ?int $logId = null): array
    {
        ...
        if ($logId) {
            \Illuminate\Support\Facades\Cache::put("otp_log_id:{$sessionToken}", $logId, now()->addMinutes($expiresMins + 5));
        }
    ...
    public function verifyOtp(string $sessionToken, string $submittedCode): array
    {
        ...
        $logId = \Illuminate\Support\Facades\Cache::pull("otp_log_id:{$sessionToken}");
        ...
        return ['success' => true, 'reason' => 'verified', 'user_id' => $otpRecord->user_id, 'log_id' => $logId];
    }
