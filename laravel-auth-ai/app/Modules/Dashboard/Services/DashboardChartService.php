...
class DashboardChartService
{
    ...
    public function getLoginActivityChart(string $period): array
    {
        return Cache::remember("dash:chart:login:{$period}", 300, function () use ($period) {
            ...
            $tzOffset = $this->getOffsetFromTz($this->timezoneService->getUserTimezone());
    ...
    {
        return Cache::remember('dash:chart:today_hourly:' . $this->timezoneService->getUserTimezone(), 60, function () {
            $userTz = $this->timezoneService->getUserTimezone();
            
            // "Hari ini" dihitung berdasarkan timezone user
            $localNow   = $this->timezoneService->toLocal(now());
