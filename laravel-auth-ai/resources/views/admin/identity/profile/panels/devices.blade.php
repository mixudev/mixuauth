...
<div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
    <div class="px-8 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-800/20 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Kelola perangkat yang diizinkan login ke akun Anda.</p>
        </div>
    ...
    <div class="divide-y divide-slate-100 dark:divide-slate-800/60">
        @forelse($devices as $device)
            ...
            @endphp
            <div class="px-8 py-5 flex flex-col sm:flex-row sm:items-center gap-4 group hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors"
                ...
                <div class="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0 shadow-sm">
                    <i class="fa-solid {{ $icon }} text-slate-500 dark:text-slate-400 text-lg"></i>
                </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex flex-wrap gap-x-5 gap-y-1 mt-1">
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                            <i class="fa-solid fa-location-dot text-[9px] text-slate-300 dark:text-slate-600"></i>
                            {{ $device->country_code ?? 'Unknown' }} · <span class="font-mono">{{ $device->ip_address }}</span>
                        </span>
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                            <i class="fa-solid fa-clock text-[9px] text-slate-300 dark:text-slate-600"></i>
                            Terakhir aktif: {{ $device->last_seen_at?->diffForHumans() ?? '—' }}
                        </span>
                        @if($device->trusted_until && !$isRevoked)
                            <span class="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                                <i class="fa-solid fa-calendar text-[9px] text-slate-300 dark:text-slate-600"></i>
                                Berlaku s/d: {{ $device->trusted_until->translatedFormat('d M Y') }}
                            </span>
                        @endif
                    </div>
                </div>
                ...
                @if(!$isRevoked)
                    <div class="shrink-0">
                        <button type="button"
                            ...
                            class="h-9 px-4 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-red-300 dark:hover:border-red-800 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 text-xs font-semibold transition-all flex items-center gap-1.5">
        ...
        @empty
            <div class="px-8 py-16 text-center">
                ...
                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Belum ada perangkat terpercaya.</p>
                <p class="text-xs text-slate-400 mt-1">Perangkat akan muncul setelah login dan verifikasi OTP berhasil.</p>
            </div>
        @endforelse
    </div>
</div>
